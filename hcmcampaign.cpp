...
Position::Position(const string &str_pos) {
    ...
    try {
        ...
        if (open == string::npos || comma == string::npos || close == string::npos) {
            r = c = 0; // Giá trị mặc định
            return;
        }
...
void ARVN::fight(Army* enemy, bool defense = false) {
        if (!enemy || !enemy->getUnitList()) return;
...
void UnitList::removeUnits() {
    ...
    while (current) {
        ...
        if (unit && unit->getQuantity() <= 0) {
            ...
            current = current->next;
            if (toDelete == tail) tail = prev; // Cập nhật tail
            if (dynamic_cast<Vehicle*>(unit)) --count_vehicle;
            else if (dynamic_cast<Infantry*>(unit)) --count_infantry;
            --size;
...
if (row < 0 || row >= n_rows || col < 0 || col >= n_cols || !army || !terrain[row][col]) return;
    terrain[row][col]->getEffect(army);
}
...
int parseIntSafe(const std::string& s, int defaultVal = 0) {
    try {
        return std::stoi(s);
    } catch (...) {
        return defaultVal;
...
void Configuration::parseUnitArray(const std::string& line, std::vector<Unit*>& liber, std::vector<Unit*>& arvn) {
        ...
        try {
            ...
            int q = parseIntSafe(content.substr(start, i - start));
            ++i;
            start = i;
            while (i < n && content[i] != ',') ++i;
            int w = parseIntSafe(content.substr(start, i - start));
            ++i;
            while (i < n && content[i] != '(') ++i;
            size_t pos_l = i;
            while (i < n && content[i] != ')') ++i;
            ...
            int side = parseIntSafe(content.substr(start, i - start));
